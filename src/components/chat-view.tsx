      ...
      <header className="mb-4 flex flex-col gap-3 border-b border-[var(--border)] pb-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex min-w-0 items-center justify-between gap-3">
          ...
          >
            ...
            <div className="user-name-shine font-mono text-sm">{handle}</div>
          </Link>
        </div>
        ...
        <div className="mb-4 rounded-md border border-[var(--border)] bg-black/3 p-3 text-sm dark:bg-white/5">
          <div className="flex items-start justify-between gap-3">
            <p className="text-[var(--muted)]">
              Your handle <span className="user-name-shine font-mono text-[var(--foreground)]">{handle}</span> lives on this device only.
        ...
        {matches.map((m) => (
          ...
          >
            <Link
              ...
              className="user-name-shine font-mono text-sm text-[var(--foreground)] hover:underline"
            >
              {m.owner_handle}
            </Link>
...
function Message({
  ...
  return (
    <div className={isUser ? "flex flex-col items-end" : "flex flex-col items-start"}>
      {showSender && (
        <span className="user-name-shine mb-0.5 text-[10px] font-mono text-[var(--muted)]">
          {senderHandle}
        </span>
      )}
      <div
